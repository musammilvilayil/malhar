  ...
  <footer className="relative bg-emerald text-cream overflow-hidden" data-testid="site-footer">
    ...
    <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-10">
      <div className="grid lg:grid-cols-12 gap-8 lg:gap-10 pb-16 border-b border-cream/15">
        <div className="lg:col-span-5">
          ...
          <div className="flex gap-4 mt-6">
            {SOCIAL_LINKS.map(({ href, label, icon: Icon }, i) => (
              <a key={i} href={href} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full border border-cream/25 flex items-center justify-center hover:bg-gold hover:text-emerald hover:border-gold transition-all duration-300 hover:-translate-y-1" data-testid={`footer-social-${i}`} aria-label={label}>
                <Icon size={17} />
              </a>
            ))}
          </div>
        ...
        <SpotlightCard className="lg:col-span-3 rounded-[1.75rem] border border-cream/10 bg-white/[0.025] p-6" spotlightColor="rgba(212,175,55,.17)">
        ...
        <SpotlightCard className="lg:col-span-4 rounded-[1.75rem] border border-cream/10 bg-white/[0.025] p-6" spotlightColor="rgba(212,175,55,.17)">
    ...
    <div className="relative overflow-hidden border-t border-cream/10 flex justify-center">
      <BlurText
        text="Malhar"
        animateBy="letters"
        delay={55}
        className="font-serif text-7xl md:text-[10rem] leading-none text-cream/5 text-center whitespace-nowrap select-none -mb-24 pt-4"
      />
    </div>
  </footer>
);
