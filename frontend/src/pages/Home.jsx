...
const Strip = () => (
  <div className="bg-gold py-6 border-y border-brass/40" data-testid="marquee">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div className="overflow-hidden"><Marquee speed={40} gradient={false} autoFill>
      {["Malhar Educational Trust", "Empowering Generations", "Tradition meets Tomorrow", "Knowledge · Faith · Character", "Islamic & Modern Education", "Community · Learning · Service"].map((t, i) => <span key={i} className="font-serif italic text-emerald text-2xl md:text-3xl mx-8 whitespace-nowrap">{t} <span className="text-brass not-italic">✦</span></span>)}
    </Marquee></div></div>
  </div>
);
...
/* ---------------- Manifesto / About ---------------- */
const Manifesto = () => (
  <section className="py-28 bg-cream border-t border-charcoal/10" data-testid="about"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-12 gap-14"><div className="lg:col-span-5"><div className="lg:sticky lg:top-28"><Reveal><div className="border border-charcoal/10 p-3"><div className="overflow-hidden aspect-[4/5] grayscale"><img src="/assets/Posoat-Thangal-360x370.jpg" alt="Portrait of founder Sheikh Sayyid Umarul Farooq Al Bukhari (Posoat Thangal)" className="w-full h-full object-cover" loading="lazy" /></div></div><p className="mt-5 font-serif text-xl text-charcoal">Sheikh Sayyid Umarul Farooq Al Bukhari</p><p className="text-sm text-gold-brass tracking-wide uppercase">Founder · Posoat Thangal</p></Reveal></div></div><div className="lg:col-span-7"><Reveal><Overline>Our Vision</Overline></Reveal><Reveal delay={0.1}><h2 className="mt-6 font-serif font-light text-4xl md:text-5xl leading-tight text-charcoal">A foundation for faith, knowledge and character.</h2></Reveal><div className="mt-12 space-y-10">{MANIFESTO.map((m, i) => <Reveal key={m.n} delay={i * 0.1}><div className="flex gap-6 border-t border-charcoal/10 pt-6"><span className="font-serif text-2xl text-gold shrink-0">{m.n}</span><div><h3 className="font-serif text-2xl text-charcoal">{m.title}</h3><p className="mt-2 text-charcoal/60 font-light leading-relaxed">{m.body}</p></div></div></Reveal>)}</div><Reveal delay={0.2}><Link to="/about-us" className="mt-10 inline-flex items-center gap-2 text-emerald font-medium link-underline" data-testid="about-readmore">Read our full story <ArrowRight size={17} /></Link></Reveal></div></div></section>
);

/* ---------------- Institutions ---------------- */
const Institutions = () => {
  const displayItems = [...institutions];
  return <section className="py-28 bg-emerald text-cream relative overflow-hidden" data-testid="institutions"><div className="absolute inset-0"><GeoPattern color="#C5A059" opacity={0.05} /></div><div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-16"><div><Reveal><Overline>Our Institutions</Overline></Reveal><Reveal delay={0.1}><h2 className="mt-5 font-serif font-light text-4xl md:text-5xl">Seven pillars of learning.</h2></Reveal></div><Reveal delay={0.2}><Link to="/our-institutions" className="inline-flex items-center gap-2 text-gold link-underline" data-testid="institutions-viewall">View all <ArrowRight size={17} /></Link></Reveal></div><Stagger className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-cream/10">{displayItems.map((it, i) => <StaggerItem key={it.slug} className={`bg-emerald ${i === 0 ? "lg:col-span-2" : ""}`}><Link to="/our-institutions" className="group block img-zoom relative h-full min-h-[320px]" data-testid={`inst-card-${it.slug}`}><img src={it.image} alt={it.name} className="absolute inset-0 w-full h-full object-cover opacity-55" loading="lazy" decoding="async" /><div className="absolute inset-0 bg-gradient-to-t from-emerald via-emerald/40 to-transparent" /><div className="relative h-full flex flex-col justify-end p-8"><h3 className="font-serif text-2xl leading-snug">{it.name}</h3><p className="mt-2 text-cream/70 text-sm font-light max-w-sm">{it.description || it.desc}</p><span className="mt-4 inline-flex items-center gap-2 text-gold text-sm opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">Learn More <ArrowUpRight size={16} /></span></div></Link></StaggerItem>)}</Stagger></div></section>;
};

/* ---------------- Campus Video ---------------- */
const CampusVideo = () => {
  const [play, setPlay] = useState(false);
  return <section className="py-28 bg-cream" data-testid="campus-video"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><Reveal className="text-center max-w-2xl mx-auto mb-14"><Overline className="justify-center">Campus Life</Overline><h2 className="mt-5 font-serif font-light text-4xl md:text-5xl text-charcoal">Step inside Malhar.</h2></Reveal><Reveal delay={0.15}><div className="relative border border-charcoal/10 p-3"><div className="relative aspect-video overflow-hidden bg-charcoal">{play ? <iframe className="w-full h-full" src={`https://www.youtube.com/embed/${youtubeChannel.latestVideoId}?autoplay=1&rel=0`} title={youtubeChannel.title} allow="autoplay; encrypted-media; fullscreen" allowFullScreen /> : <><img src="/assets/gal.jpg" alt="Malhar campus cover" className="w-full h-full object-cover opacity-60" loading="lazy" decoding="async" /><div className="absolute inset-0 bg-gradient-to-t from-emerald/85 via-emerald/15 to-emerald/40" /><span className="absolute top-5 left-5 inline-flex items-center gap-1.5 bg-gold text-emerald text-xs font-semibold tracking-[0.15em] uppercase px-3 py-1.5" data-testid="video-badge"><Film size={13} /> Campus Film</span><span className="absolute top-5 right-5 inline-flex items-center gap-1.5 bg-black/40 backdrop-blur-sm text-cream text-xs px-3 py-1.5 border border-cream/25" data-testid="video-duration"><Clock size={12} /> Full Tour</span><button onClick={() => setPlay(true)} className="absolute inset-0 flex items-center justify-center group" data-testid="video-play" aria-label="Play campus tour"><span className="w-20 h-20 rounded-full bg-gold text-emerald flex items-center justify-center group-hover:scale-110 transition-transform"><Play size={28} className="ml-1" fill="currentColor" /></span></button><div className="absolute bottom-6 left-6 right-6 pointer-events-none"><p className="text-gold text-xs tracking-[0.2em] uppercase">Watch the film</p><h3 className="mt-1 font-serif text-2xl md:text-3xl text-cream">Inside Malhar</h3></div></>}</div></div></Reveal></div></section>;
};

const MediaHighlight = () => {
  const videoId = youtubeChannel.latestVideoId;
  const previewSrc = `/assets/${videoId}-maxresdefault.jpg`;
  return <section className="py-28 bg-gold/5 border-t border-charcoal/10" data-testid="media-highlight"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid gap-12 lg:grid-cols-2 items-center"><div><Reveal><Overline>Media Highlight</Overline></Reveal><Reveal delay={0.1}><h2 className="mt-5 font-serif font-light text-4xl md:text-5xl text-charcoal">Explore Malhar Media on YouTube.</h2></Reveal><Reveal delay={0.2}><p className="mt-6 text-charcoal/70 font-light text-lg max-w-xl">Watch lectures, campus updates, events and community stories from Malhar.</p></Reveal><Reveal delay={0.3}><div className="mt-8 flex flex-wrap gap-4"><Link to="/media" className="inline-flex items-center gap-2 px-6 py-4 bg-emerald text-cream hover:bg-emerald-light transition-colors">Explore Media</Link></div></Reveal></div><Reveal delay={0.15}><div className="relative overflow-hidden border border-charcoal/10 bg-charcoal aspect-video"><img src={previewSrc} alt={`${youtubeChannel.title} preview`} className="w-full h-full object-cover" loading="lazy" decoding="async" /><div className="absolute inset-0 bg-gradient-to-t from-emerald/80 via-transparent to-transparent" /><div className="absolute inset-x-0 bottom-0 p-6"><p className="text-sm uppercase tracking-[0.2em] text-cream/80">YouTube Channel</p><h3 className="mt-2 font-serif text-3xl text-cream">{youtubeChannel.title}</h3><p className="mt-3 text-sm text-cream/70 max-w-sm">{youtubeChannel.description}</p></div></div></Reveal></div></section>;
};

const Personalities = () => <section className="py-28 bg-cream border-t border-charcoal/10" data-testid="personalities"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><Reveal className="mb-14"><Overline>Key Personalities</Overline><h2 className="mt-5 font-serif font-light text-4xl md:text-5xl text-charcoal">The people behind Malhar.</h2></Reveal><Stagger className="grid grid-cols-2 lg:grid-cols-4 gap-6">{instructors.map((p) => <StaggerItem key={p.slug}><Link to={`/instructor/${p.slug}`} className="group block" data-testid={`person-${p.slug}`}><div className="img-zoom border border-charcoal/10 aspect-[3/4] grayscale group-hover:grayscale-0 transition-all duration-500">{p.image ? <img src={p.image} alt={p.name} className="w-full h-full object-cover" loading="lazy" decoding="async" /> : <div className="w-full h-full bg-emerald/10 flex items-center justify-center font-serif text-5xl text-emerald/40">{p.name?.charAt(0)}</div>}</div><p className="mt-4 text-xs uppercase tracking-[0.15em] text-gold-brass">{p.role}</p><p className="mt-1 font-serif text-lg text-charcoal leading-snug">{p.name}</p></Link></StaggerItem>)}</Stagger></div></section>;
